...
function createApplication(options) {
  ...
  app.response.__proto__.ok = function(ret){
    this.json({status:1 , result:ret});
  };
  app.response.__proto__.err = function(code,msg){
    this.json({status:code , result:{msg:msg}});
  };
  app.response.__proto__.errWithCode = function(s,c){
    this.json({status:s, code:c});
  };
...
loopback.findAndRemoveModel = function(modelName) {
  return this.registry.findAndRemoveModel.apply(this.registry, arguments);
};
/**
 * Look up a model class by name from all models created by
